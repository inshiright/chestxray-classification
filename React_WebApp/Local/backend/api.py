...
def _get_attention_heatmap(input_tensor: torch.Tensor):
    ...
    """
    # Always reset output_attentions to False after we're done,
    # so subsequent _infer_probs / predict calls don't carry the flag.
    ...
    # FIX: resize to the actual spatial size of input_tensor (518 for RadDINO),
    # NOT the global IMAGE_SIZE constant (224).  np_img is produced at
    # _model_input_size() so heatmap must match that or cv2.addWeighted crashes.
    spatial = input_tensor.shape[-1]  # input_tensor is (1, 3, H, W)
    heatmap = cv2.resize(attn_map, (spatial, spatial))
...
def _get_radjepa_attention_heatmap(input_tensor: torch.Tensor):
    ...
    Returns (heatmap np float32 [H,W], probs np float32 [C]).
    """
    # timm's Attention.forward() calls softmax as a functional op inline —
    # there is no nn.Softmax submodule to hook, so block.attn.softmax does
    # not exist and raises AttributeError.
    # block.attn.attn_drop IS a real nn.Dropout submodule that sits immediately
    # after softmax.  We use a forward_pre_hook to capture its INPUT (which is
    # the post-softmax attention matrix) before any dropout is applied.
    # This works correctly regardless of dropout rate (including rate=0).
    hooks = []
    attn_maps = []

    def make_hook(attn_maps_list):
        def hook_fn(module, input):
            # input is a tuple; input[0] shape: [B, heads, N, N] — post-softmax
            attn_maps_list.append(input[0].detach().clone())
        return hook_fn

    for block in MODEL.encoder.model.blocks:
        hook = block.attn.attn_drop.register_forward_pre_hook(make_hook(attn_maps))
        hooks.append(hook)
